import{g as f,i as b}from"./assets/icon-b601c114.js";import"./assets/vendor-611745ce.js";const S="/project-js-dreamteam/assets/yellowBasket-88cc64a6.png",k={FILTERS_KEY:"filters",CART_KEY:"cart"},E=document.querySelector("span#cart-counter"),L=document.querySelector(".js-cart-block"),u=document.querySelector(".cart-list-block"),w=document.querySelector(".cart-delete-btn");w.addEventListener("click",$);function $(){localStorage.removeItem("cart"),d()}function m(t){try{const e=localStorage.getItem(t);return e?JSON.parse(e):[]}catch(e){console.log(e.message)}}async function d(){let t=await m(k.CART_KEY);if(E.textContent=t.length,t.length===0){L.innerHTML=I();return}x()}d();async function x(){const e=m("cart").map(a=>a._id);u.innerHTML="";try{let a=0,o=[],r=[],c=[];for(let s=0;s<e.length;s+=1){const i=await f(e[s]),g=_(i,e[s]);u.innerHTML+=g;const p=JSON.parse(localStorage.getItem("cart"));a+=i.price,o.push(i.price),q.textContent=`${Number(a.toFixed(2))}`,C(".cart-close").then(l=>{c.push(p[s]._id),localStorage.setItem("cart1",JSON.stringify(r)),l.forEach(y=>{y.addEventListener("click",v=>{const h=v.currentTarget.id;c=c.filter(n=>n!==h),r=[];for(let n=0;n<c.length;n+=1)r.push({_id:c[n],quantity:1});localStorage.setItem("cart",JSON.stringify(r)),d()})})}).catch(l=>{console.error(l.message)})}}catch(a){console.log(a)}}function C(t){return new Promise((e,a)=>{const o=document.querySelectorAll(t);if(o.length>0)e(o);else{const r=new MutationObserver(()=>{const c=document.querySelectorAll(t);c.length>0&&(r.disconnect(),e(c))});r.observe(document.documentElement,{childList:!0,subtree:!0})}})}document.addEventListener("DOMContentLoaded",function(){document.getElementById("cart"),window.addEventListener("scroll",function(){})});function _(t){return t.category.includes("_")&&(t.category=t.category.replace(/_/g," ")),`
