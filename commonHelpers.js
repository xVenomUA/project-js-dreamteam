...
`).join("");u.filtercard.innerHTML=r}async function g(){try{const t=await f();t.push("Show all");const r=t.map(o=>`<option value="${o}">${o}</option>`).join("");u.categor.innerHTML+=r}catch(t){console.log(t)}}async function y(){try{const r=(await d("","","","","",1)).results;console.log(r),p(r)}catch(t){console.log(t)}}g();y();const u={categor:document.querySelector(".filter-categories"),filtercard:document.querySelector(".filter-cards")};
