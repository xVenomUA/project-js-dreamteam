import{o as u,g as S,i as E,p as L}from"./assets/subscription-d9763ab2.js";import"./assets/vendor-611745ce.js";const w="/project-js-dreamteam/assets/yellowBasket-88cc64a6.png",C={FILTERS_KEY:"filters",CART_KEY:"cart"},q=document.querySelector("span#cart-counter"),I=document.querySelector(".js-cart-block"),y=document.querySelector(".cart-list-block"),$=document.querySelector(".cart-delete-btn"),x=document.querySelector(".cart-form"),m=document.querySelector(".modal-backdrop-two");$.addEventListener("click",_);function _(){localStorage.removeItem("cart"),l()}function g(t){try{const e=localStorage.getItem(t);return e?JSON.parse(e):[]}catch(e){console.log(e.message)}}async function l(){let t=await g(C.CART_KEY);if(q.textContent=t.length,t.length===0){I.innerHTML=A(),u();return}M()}l();async function M(){const e=g("cart").map(c=>c._id);y.innerHTML="";try{let c=0,s=[],a=[],r=[];for(let o=0;o<e.length;o+=1){const i=await S(e[o]),v=B(i,e[o]);y.innerHTML+=v;const f=JSON.parse(localStorage.getItem("cart"));c+=i.price,s.push(i.price),T.textContent=`${Number(c.toFixed(2))}`,O(".cart-close").then(d=>{r.push(f[o]._id),localStorage.setItem("cart1",JSON.stringify(a)),d.forEach(h=>{h.addEventListener("click",b=>{const k=b.currentTarget.id;r=r.filter(n=>n!==k),a=[];for(let n=0;n<r.length;n+=1)a.push({_id:r[n],quantity:1});localStorage.setItem("cart",JSON.stringify(a)),l()})})}).catch(d=>{console.error(d.message)})}}catch(c){console.log(c)}}function O(t){return new Promise((e,c)=>{const s=document.querySelectorAll(t);if(s.length>0)e(s);else{const a=new MutationObserver(()=>{const r=document.querySelectorAll(t);r.length>0&&(a.disconnect(),e(r))});a.observe(document.documentElement,{childList:!0,subtree:!0})}})}document.addEventListener("DOMContentLoaded",function(){document.getElementById("cart"),window.addEventListener("scroll",function(){})});function B(t){return t.category.includes("_")&&(t.category=t.category.replace(/_/g," ")),`
  ...
  `}const T=document.querySelector("span#your-order-total-price");x.addEventListener("submit",j);async function j(t){t.preventDefault();const e=document.querySelector(".error-message"),c=document.querySelector(".cart-email-input").value;u();const s=g("cart"),a=z(c,s);try{const r=await L(a);p(!0),document.body.style.overflow="hidden",localStorage.removeItem("cart"),u(),l()}catch{e.style.display="block",setTimeout(()=>e.style.display="none",2e3)}}function z(t,e){return{email:t,products:e.map(c=>({productId:c._id,amount:c.quantity}))}}function p(t=!0){m.classList.toggle("is-hidden-basket",!t),t&&D(),document.body.style.overflow="scroll"}function D(){document.querySelector(".modal-close-btn-basket").addEventListener("click",()=>p(!1)),m.addEventListener("click",e=>{e.target===m&&p(!1)})}
