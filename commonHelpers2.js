...
`).join("");a.filtercard.innerHTML=t}const h={keyword:null},c={keyword:null,category:null,page:1,limit:6};window.addEventListener("resize",y);async function E(){try{const t=(await L()).map(s=>`<option value="${s}">${s}</option>`).join("");a.categor.innerHTML+=t}catch(e){console.log(e)}}async function y(){const e=N();c.limit=e;const t=JSON.parse(localStorage.getItem("filters"));t&&(c.keyword=t.keyword,c.category=t.category,c.page=t.page,localStorage.setItem("filters",JSON.stringify(c)),w());try{const o=(await k(c.keyword,c.category,"","","",c.page,e)).results;q(o)}catch(s){console.log(s)}}E();y();a.form&&(a.form.addEventListener("input",M),a.form.addEventListener("submit",O));async function M(){const e=a.filtersInput.value;h.keyword=e,e===""&&(h.keyword=null),localStorage.setItem("keyword",JSON.stringify(h))}async function O(e){e.preventDefault();const t=a.filtersInput.value;c.keyword=t,t===""&&(c.keyword=null);const s=a.filtersCategories.value;c.category=s,s===""&&(c.category=null),s==="Show all"&&(c.category=null),localStorage.setItem("filters",JSON.stringify(c)),y()}function w(){try{const e=JSON.parse(localStorage.getItem("keyword"));a.filtersInput&&(a.filtersInput.value=e.keyword||"")}catch{return}}w();function N(){const e=window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth;return e>=1440?9:e>=768?8:6}const T=document.querySelector(".backdrop-div-card");function z(e){const{_id:t,name:s,category:o,size:r,popularity:i,desc:n,price:l,img:m}=e,S=`<div class="shop-modal-backdrop">
<div class="shop-div-card" data-idcarts="${t}">
  ...
  <img class="shop-img" src="${m}" alt="${s}" width="140">
  </div>
