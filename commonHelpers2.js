import{i as p,r as n,a as B,g as $,o as g,A as W,b as z,c as R}from"./assets/subscription-d9763ab2.js";import{l as j}from"./assets/vendor-611745ce.js";function C(s,e){document.querySelectorAll(`[data-idcarts="${s}"] .filt-use`).forEach(o=>{o.setAttribute("href",e)})}function F(s){const e=s.map(({_id:t,category:o,img:c,name:a,size:r,popularity:i,price:l})=>{o.includes("_")&&(o=o.replace(/_/g," "));let u="",y="shop";const m=JSON.parse(localStorage.getItem("cart"));return m&&m.find(v=>v._id===t)&&(y="check",u="disabled"),`<div class="filt-div-card" data-idcarts="${t}">
...
`}).join("");n.filtercard.innerHTML=e,n.filtercard.innerHTML===""&&(n.filtercard.innerHTML=`<div class="filt"><p class="filt-text-no">Nothing was found for the selected <a href="#" class="filt-span-no"> filters...</a></p>
    <p class="filt-p-no">Try adjusting your search parameters or browse our range by other criteria to find the perfect product for you. </p>
          ...
          <use href="${p}#icon-shop"></use>
        </svg>`,u.setAttribute("data-check","false"),u.classList.remove("shop-btn-card-active")),u.dataset.idcards=e,y.textContent=t,m.textContent=o,h.textContent=c,v.textContent=a,T.textContent=r,H.textContent=`$${i}`,L.src=l,L.alt=t}function Z(s){const e=s.map(({_id:t,category:o,img:c,name:a,size:r,popularity:i})=>{o.includes("_")&&(o=o.replace(/_/g," "));let l="shop",u="";const y=JSON.parse(localStorage.getItem("cart"));return y&&y.find(h=>h._id===t)&&(l="check",u="popular-icon-back"),`
...
`}).join("");n.DiscountDiv.innerHTML=e}nt();async function nt(){try{const t=(await R()).data;A(t);const o=s(t,2);A(o)}catch{}function s(e,t){return e.sort(()=>.5-Math.random()).slice(0,t)}}n.DiscountDiv.addEventListener("click",lt);async function lt(s){s.preventDefault(),document.body.style.overflow="hidden";const{target:e}=s,t=e.closest(".product-card");if(!t||e.closest(".discount-icon-cont"))return;const c=t.dataset.idcarts;try{const a=await $(c);O(a),n.modalWindowId.classList.remove("is-hidden-card")}catch(a){console.log(a)}}async function dt(s){try{const{target:e}=s,t=e.closest(".discount-icon-cont");if(!t)return;t.querySelector(".filt-use").setAttribute("href",`${p}#icon-check`),t.setAttribute("disabled","true");const c=t.dataset.idcarts,a=localStorage.getItem("cart"),r=JSON.parse(a)||[],i=r.find(l=>l._id===c);i?i.quantity+=1:(r.push({_id:c,quantity:1}),C(c,`${p}#icon-check`),g()),localStorage.setItem("cart",JSON.stringify(r))}catch{}document.body.style.overflow="scroll"}n.DiscountDiv.addEventListener("click",dt);const ut=document.querySelector(".shop-modal-close-btn");n.listCards.addEventListener("click",pt);async function pt(s){s.preventDefault();const{target:e}=s,t=e.closest(".filt-div-card");if(!t||e.closest(".filt-btn-card"))return;const c=t.dataset.idcarts;try{const a=await $(c);O(a),n.modalWindowId.classList.remove("is-hidden-card")}catch(a){console.log(a)}document.body.style.overflow="hidden"}ut.addEventListener("click",M);function M(){n.modalWindowId.classList.add("is-hidden-card"),document.body.style.overflow="scroll"}function ft(s){const{target:e}=s,t=e.closest(".shop-btn-card");if(f(),P(),!t)return;const o=t.dataset.idcards,c=t.dataset.check;console.log(c),M();const a=localStorage.getItem("cart"),r=JSON.parse(a);if(c==="false"){if(r){const l=r.find(u=>u._id===o);if(l){l.quantity+=1,localStorage.setItem("cart",JSON.stringify(r));return}r.push({_id:o,quantity:1}),localStorage.setItem("cart",JSON.stringify(r)),g();return}const i=[];i.push({_id:o,quantity:1}),localStorage.setItem("cart",JSON.stringify(i)),g()}else if(r&&r.find(l=>l._id===o)){const l=r.filter(u=>u._id!==o);localStorage.setItem("cart",JSON.stringify(l)),g();return}}n.addCartBtn.addEventListener("click",ft);
