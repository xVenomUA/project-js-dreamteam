...
`)).join("");i.filtercard.innerHTML=t}const g={keyword:null},c={keyword:null,category:null,page:1,limit:6};window.addEventListener("resize",v);async function F(){try{const t=(await M()).map(s=>{let r;return s.includes("_")?r=s.replace(/_/g," "):r=s,`<option value="${s}">${r}</option>`}).join("");i.categor.innerHTML+=t;const o=document.createElement("option");o.value="Show all",o.textContent="Show all",i.categor.prepend(o)}catch(e){console.log(e)}}async function v(){const e=R();c.limit=e;const t=JSON.parse(localStorage.getItem("filters"));t&&(c.keyword=t.keyword,c.category=t.category,c.page=t.page,localStorage.setItem("filters",JSON.stringify(c)),$());try{const s=(await _(c.keyword,c.category,"","","",c.page,e)).results;A(s)}catch(o){console.log(o)}}F();v();i.form&&(i.form.addEventListener("input",T),i.form.addEventListener("submit",D));async function T(){const e=i.filtersInput.value;g.keyword=e,e===""&&(g.keyword=null),localStorage.setItem("keyword",JSON.stringify(g))}async function D(e){e.preventDefault();const t=i.filtersInput.value;c.keyword=t,t===""&&(c.keyword=null);const o=i.filtersCategories.value;c.category=o,o===""&&(c.category=null),o==="Show all"&&(c.category=null),localStorage.setItem("filters",JSON.stringify(c)),v()}function $(){try{const e=JSON.parse(localStorage.getItem("keyword"));i.filtersInput&&(i.filtersInput.value=e.keyword||"")}catch{return}}$();function R(){const e=window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth;return e>=1440?9:e>=768?8:6}function U(e){const{_id:t,name:o,category:s,size:r,popularity:n,desc:a,price:l,img:d}=e,k=document.querySelector(".shop-btn-card"),P=document.querySelector(".shop-h-name"),q=document.querySelector(".shop-span-category"),I=document.querySelector(".shop-span-size"),x=document.querySelector(".shop-span-popularity"),E=document.querySelector(".shop-text-discription"),O=document.querySelector(".shop-h-price"),S=document.querySelector(".shop-img");k.dataset.idcards=t,P.textContent=o,q.textContent=s,I.textContent=r,x.textContent=n,E.textContent=a,O.textContent=`$${l}`,S.src=d,S.alt=o}const j=document.querySelector(".filter-cards"),C=document.getElementById("id-modal-card"),V=document.querySelector(".shop-modal-close-btn"),W=document.querySelector(".shop-btn-card");j.addEventListener("click",H);async function H(e){e.preventDefault();const{target:t}=e,o=t.closest(".filt-btn-card");if(!o)return;const s=o.dataset.idcards;try{const r=await N(s);U(r),C.classList.remove("is-hidden-card")}catch(r){console.log(r)}}V.addEventListener("click",L);function L(){C.classList.add("is-hidden-card")}function G(e){const{target:t}=e,o=t.closest(".shop-btn-card");if(!o)return;const s=o.dataset.idcards;L();const r=localStorage.getItem("cart"),n=JSON.parse(r);if(n){const l=n.find(d=>d._id===s);if(l){l.quantity+=1,localStorage.setItem("cart",JSON.stringify(n));return}n.push({_id:s,quantity:1}),localStorage.setItem("cart",JSON.stringify(n));return}const a=[];a.push({_id:s,quantity:1}),localStorage.setItem("cart",JSON.stringify(a))}W.addEventListener("click",G);function K(e){return e.category.includes("_")&&(e.category=e.category.replace(/_/g," ")),`
