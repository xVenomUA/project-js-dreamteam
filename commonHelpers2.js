import{i as p,a as A,g as x,A as B,b as H,c as D,p as W}from"./assets/icon-b601c114.js";import{l as V}from"./assets/vendor-611745ce.js";const n={categor:document.querySelector(".filter-categories"),filtercard:document.querySelector(".filter-cards"),form:document.querySelector(".filter-div-label"),filtersInput:document.getElementById("filter-search"),filtersCategories:document.querySelector(".filter-categories"),addCartBtn:document.querySelector(".shop-btn-card"),listCards:document.querySelector(".filter-cards"),countHeader:document.querySelector(".header-cart-count"),PopularDiv:document.getElementById("popularContainer"),modalWindowId:document.getElementById("id-modal-card")};function f(){const t=JSON.parse(localStorage.getItem("cart"));t?n.countHeader.textContent=`(${t.length})`:n.countHeader.textContent="(0)"}f();function z(t){const e=t.map(({_id:s,category:o,img:i,name:c,size:a,popularity:r,price:l})=>{o.includes("_")&&(o=o.replace(/_/g," "));let d="",m="shop";const g=JSON.parse(localStorage.getItem("cart"));return g&&g.find(S=>S._id===s)&&(m="check",d="disabled"),`<div class="filt-div-card" data-idcarts="${s}">
...
`}).join("");n.filtercard.innerHTML=e,n.filtercard.innerHTML===""&&(n.filtercard.innerHTML=`<div class="filt"><p class="filt-text-no">Nothing was found for the selected <a href="#" class="filt-span-no"> filters...</a></p>
    <p class="filt-p-no">Try adjusting your search parameters or browse our range by other criteria to find the perfect product for you. </p>
          ...
          <use href="${p}#icon-shop"></use>
        </svg>`,d.setAttribute("data-check","false"),d.classList.remove("shop-btn-card-active")),d.dataset.idcards=e,m.textContent=s,g.textContent=o,y.textContent=i,S.textContent=c,_.textContent=a,M.textContent=`$${r}`,L.src=l,L.alt=s}function F(t){const e=t.map(({_id:s,category:o,img:i,name:c,size:a,popularity:r})=>{o.includes("_")&&(o=o.replace(/_/g," "));let l="shop",d="";const m=JSON.parse(localStorage.getItem("cart"));return m&&m.find(y=>y._id===s)&&(l="check",d="popular-icon-back"),`
...
`}).join("");n.PopularDiv.innerHTML=e}w();async function w(){try{const e=(await A()).data;F(e)}catch{}}n.PopularDiv.addEventListener("click",j);async function j(t){t.preventDefault();const{target:e}=t,s=e.closest(".popular-card");if(!s||e.closest(".popular-icon-cont"))return;const i=s.dataset.idcarts;try{const c=await x(i);$(c),n.modalWindowId.classList.remove("is-hidden-card")}catch(c){console.log(c)}}function R(t){v();const{target:e}=t,s=e.closest(".popular-icon-cont");if(!s)return;s.querySelector(".filt-use").setAttribute("href",`${p}#icon-check`),s.setAttribute("disabled","true"),s.classList.add("popular-icon-back");const c=s.dataset.idcarts,a=localStorage.getItem("cart"),r=JSON.parse(a);if(r){const l=r.find(d=>d._id===c);if(l){l.quantity+=1,localStorage.setItem("cart",JSON.stringify(r));return}r.push({_id:c,quantity:1}),localStorage.setItem("cart",JSON.stringify(r)),f();return}if(!r){const l=[];l.push({_id:c,quantity:1}),localStorage.setItem("cart",JSON.stringify(l)),f()}}n.PopularDiv.addEventListener("click",R);const b={keyword:null},u={keyword:null,category:null,page:1,limit:6};window.addEventListener("resize",V(v,2500));async function U(){try{const e=(await B()).map(o=>{let i;return o.includes("_")?i=o.replace(/_/g," "):i=o,`<option value="${o}">${i}</option>`}).join("");n.categor.innerHTML+=e;const s=document.createElement("option");s.value="Show all",s.textContent="Show all",n.categor.prepend(s)}catch(t){console.log(t)}}async function v(){const t=Q();u.limit=t;const e=JSON.parse(localStorage.getItem("filters"));e&&(u.keyword=e.keyword,u.category=e.category,u.page=e.page,localStorage.setItem("filters",JSON.stringify(u)),E());try{const s=await H(u.keyword,u.category,"","","",u.page,t);localStorage.setItem("totalPage",JSON.stringify(s.totalPages));const o=s.results;z(o)}catch(s){console.log(s)}}U();v();n.form&&(n.form.addEventListener("input",G),n.form.addEventListener("submit",K));async function G(){const t=n.filtersInput.value;b.keyword=t,t===""&&(b.keyword=null),localStorage.setItem("keyword",JSON.stringify(b))}async function K(t){t.preventDefault();const e=n.filtersInput.value;u.keyword=e,e===""&&(u.keyword=null);const s=n.filtersCategories.value;u.category=s,s===""&&(u.category=null),s==="Show all"&&(u.category=null),localStorage.setItem("filters",JSON.stringify(u)),v(),t.target.reset()}function E(){try{const t=JSON.parse(localStorage.getItem("keyword"));n.filtersInput&&(n.filtersInput.value=t.keyword||"")}catch{return}}E();function Q(){const t=window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth;return t>=1440?9:t>=768?8:6}function X(t){w();const{target:e}=t,s=e.closest(".filt-btn-card");if(!s)return;s.querySelector(".filt-use").setAttribute("href",`${p}#icon-check`),s.setAttribute("disabled","true");const i=s.dataset.idcards,c=localStorage.getItem("cart"),a=JSON.parse(c);if(a){const r=a.find(l=>l._id===i);if(r){r.quantity+=1,localStorage.setItem("cart",JSON.stringify(a));return}a.push({_id:i,quantity:1}),localStorage.setItem("cart",JSON.stringify(a)),f();return}if(!a){const r=[];r.push({_id:i,quantity:1}),localStorage.setItem("cart",JSON.stringify(r)),f()}}n.listCards.addEventListener("click",X);const N=document.querySelector(".pagination ul");let Y=10,Z=1;const tt=document.querySelector(".pagination");N.innerHTML=et(Y,Z);function et(t,e){let s="",o,i=e-1,c=e+1;e>1&&(s+=`<li class="btn prev" onclick="createPagination(totalPages, ${e-1})"><span><i class="fas fa-angle-left"></i> Prev</span></li>`),e>2&&(s+='<li class="first numb" onclick="createPagination(totalPages, 1)"><span>1</span></li>',e>3&&(s+='<li class="dots"><span>...</span></li>')),e==t?i=i-2:e==t-1&&(i=i-1),e==1?c=c+2:e==2&&(c=c+1);for(var a=i;a<=c;a++)a>t||(a==0&&(a=a+1),e==a?o="active":o="",s+=`<li class="numb ${o}" onclick="createPagination(totalPages, ${a})"><span>${a}</span></li>`);return e<t-1&&(e<t-2&&(s+='<li class="dots"><span>...</span></li>'),s+=`<li class="last numb" onclick="createPagination(totalPages, ${t})"><span>${t}</span></li>`),e<t&&(s+=`<li class="btn next" onclick="createPagination(totalPages, ${e+1})"><span>Next <i class="fas fa-angle-right"></i></span></li>`),t===1&&(tt.style.display="none"),N.innerHTML=s,s}const st=document.getElementById("id-modal-card");function ot(t){const e=document.createElement("div");return e.classList.add("product-card"),e.dataset.idcarts=t._id,e.addEventListener("click",()=>{$(t),st.classList.remove("is-hidden-card")}),e.innerHTML=`
