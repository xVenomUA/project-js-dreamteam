...
`}).join("");n.filtercard.innerHTML=s,n.filtercard.innerHTML===""&&(n.filtercard.innerHTML=`<div class="filt"><p class="filt-text-no">Nothing was found for the selected <a href="#" class="filt-span-no"> filters...</a></p>
    <p class="filt-p-no">Try adjusting your search parameters or browse our range by other criteria to find the perfect product for you. </p>
    </div>`)}const I=document.querySelector(".pagination-list"),J=document.querySelector(".pagination"),h=document.querySelector(".last-total-page"),C=document.querySelector(".pag-first-page"),k=document.querySelector(".prew-button"),w=document.querySelector(".next-button");function F(e,s){let t="",r=e-2,a=s;if(h.textContent=e,e===1?J.style="display: none":J.style="display: flex",s==1?k.setAttribute("disabled",!0):k.removeAttribute("disabled"),s==e?w.setAttribute("disabled",!0):w.removeAttribute("disabled"),e>6){s>e-4&&(s=e-4),s===1&&(s=s+1);for(let o=s-1;o<=s+1;o++)o==a?t+=`<li class="pagination-item"><button class="pagination-number active" type="button">${o}</button></li>`:t+=`<li class="pagination-item"><button class="pagination-number" type="button">${o}</button></li>`;t+='<li class="pagination-item dot-item"><span>...</span></li>';for(let o=r;o<=e;o++)o==a?t+=`<li class="pagination-item"><button class="pagination-number active" type="button">${o}</button></li>`:t+=`<li class="pagination-item"><button class="pagination-number" type="button">${o}</button></li>`}if(I.innerHTML=t,e>1&&e<7){C.style="display: none",h.style="display: none";for(let o=1;o<=e;o++)t+=`<li class="pagination-item"><button class="pagination-number" type="button">${o}</button></li>`}else C.style="display: block",h.style="display: block";I.innerHTML=t}async function U(e){if(e.target.classList.contains("pagination-arrow")||e.target.classList.contains("pagination-number")){const s=e.target.textContent,t=JSON.parse(localStorage.getItem("filters"));t.page=parseInt(s),localStorage.setItem("filters",JSON.stringify(t)),p()}else return}async function G(e){const s=JSON.parse(localStorage.getItem("filters"));s.totalPages,s.page=s.page-1,localStorage.setItem("filters",JSON.stringify(s)),p()}async function K(e){const s=JSON.parse(localStorage.getItem("filters"));s.totalPages,s.page=Number(s.page)+1,localStorage.setItem("filters",JSON.stringify(s)),p()}async function Q(e){const s=JSON.parse(localStorage.getItem("filters"));s.page=1,localStorage.setItem("filters",JSON.stringify(s)),p()}async function X(e){const s=JSON.parse(localStorage.getItem("filters"));s.page=h.textContent,localStorage.setItem("filters",JSON.stringify(s)),p()}I.addEventListener("click",U);k.addEventListener("click",G);w.addEventListener("click",K);C.addEventListener("click",Q);h.addEventListener("click",X);const b={keyword:null},d={keyword:null,category:null,page:1,limit:A()},Y=document.querySelector(".filter-categories");localStorage.getItem("filters")||localStorage.setItem("filters",JSON.stringify(d));window.addEventListener("resize",R(p,1e3));let q=0;async function Z(){try{const s=(await H()).map(r=>{let a;return r.includes("_")?a=r.replace(/_/g," "):a=r,`<option value="${r}">${a}</option>`}).join("");n.categor.innerHTML+=s;const t=document.createElement("option");t.value="Show all",t.textContent="Show all",n.categor.prepend(t)}catch(e){console.log(e)}}async function p(){const e=A();d.limit=e;const s=JSON.parse(localStorage.getItem("filters"));s&&(d.keyword=s.keyword,d.category=s.category,d.page=s.page,localStorage.setItem("filters",JSON.stringify(d)),E());try{const t=await V(d.keyword,d.category,"","","",d.page,e);localStorage.setItem("totalPage",JSON.stringify(t.totalPages));const r=t.results;q=t.totalPages,F(q,d.page),j(r)}catch(t){console.log(t)}}Z();p();n.form&&(n.form.addEventListener("input",tt),n.form.addEventListener("submit",et));async function tt(){const e=n.filtersInput.value;b.keyword=e,e===""&&(b.keyword=null),localStorage.setItem("keyword",JSON.stringify(b))}async function et(e){e.preventDefault();const s=n.filtersInput.value;d.keyword=s,s===""&&(d.keyword=null);const t=n.filtersCategories.value;d.category=t,t===""&&(d.category=null),t==="Show all"&&(d.category=null),d.page=1,localStorage.setItem("filters",JSON.stringify(d)),p(),e.target.reset()}function E(){try{const e=JSON.parse(localStorage.getItem("keyword"));n.filtersInput&&(n.filtersInput.value=e.keyword||"")}catch{return}}E();function A(){const e=window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth;return e>=1440?9:e>=768?8:6}function st(e){const{target:s}=e,t=s.closest(".filt-btn-card");if(!t)return;t.querySelector(".filt-use").setAttribute("href",`${g}#icon-check`),t.setAttribute("disabled","true");const a=t.dataset.idcards,o=localStorage.getItem("cart"),c=JSON.parse(o);if(c){const i=c.find(l=>l._id===a);if(i){i.quantity+=1,localStorage.setItem("cart",JSON.stringify(c));return}c.push({_id:a,quantity:1}),localStorage.setItem("cart",JSON.stringify(c)),f();return}if(!c){const i=[];i.push({_id:a,quantity:1}),localStorage.setItem("cart",JSON.stringify(i)),f()}}n.listCards.addEventListener("click",st);Y.addEventListener("change",ot);function ot(){const e=n.filtersCategories.value;d.category=e,d.page=1,e===""&&(d.category=null),e==="Show all"&&(d.category=null),localStorage.setItem("filters",JSON.stringify(d)),p()}function O(e){const{_id:s,name:t,category:r,size:a,popularity:o,desc:c,price:i,img:l}=e,u=document.querySelector(".shop-btn-card"),m=document.querySelector(".shop-h-name"),y=document.querySelector(".shop-span-category"),S=document.querySelector(".shop-span-size"),v=document.querySelector(".shop-span-popularity"),M=document.querySelector(".shop-text-discription"),T=document.querySelector(".shop-h-price"),P=document.querySelector(".shop-img"),L=JSON.parse(localStorage.getItem("cart"));let N={};L&&(N=L.find(B=>B._id===s)??!1),N!==!1?(u.innerHTML=`Remove from<svg class="shop-card-icon" height="18" width="18">
          ...
          <use href="${g}#icon-shop"></use>
        </svg>`,u.setAttribute("data-check","false"),u.classList.remove("shop-btn-card-active")),u.dataset.idcards=s,m.textContent=t,y.textContent=r,S.textContent=a,v.textContent=o,M.textContent=c,T.textContent=`$${i}`,P.src=l,P.alt=t}function rt(e){const s=e.map(({_id:t,category:r,img:a,name:o,size:c,popularity:i})=>{r.includes("_")&&(r=r.replace(/_/g," "));let l="shop",u="";const m=JSON.parse(localStorage.getItem("cart"));return m&&m.find(S=>S._id===t)&&(l="check",u="popular-icon-back"),`
