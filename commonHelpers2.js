...
`}).join("");a.filtercard.innerHTML=e,a.filtercard.innerHTML===""&&(a.filtercard.innerHTML=`<div class="filt"><p class="filt-text-no">Nothing was found for the selected <a href="#" class="filt-span-no"> filters...</a></p>
    ...
    </div>`)}function V(t){const{_id:e,name:s,category:o,size:r,popularity:n,desc:c,price:l,img:d}=t,f=document.querySelector(".shop-btn-card"),h=document.querySelector(".shop-h-name"),y=document.querySelector(".shop-span-category"),$=document.querySelector(".shop-span-size"),S=document.querySelector(".shop-span-popularity"),E=document.querySelector(".shop-text-discription"),N=document.querySelector(".shop-h-price"),L=document.querySelector(".shop-img"),k=JSON.parse(localStorage.getItem("cart"));k&&k.find(M=>M._id===e)&&f.setAttribute("disabled","true"),f.dataset.idcards=e,h.textContent=s,y.textContent=o,$.textContent=r,S.textContent=n,E.textContent=c,N.textContent=`$${l}`,L.src=d,L.alt=s}const I=document.getElementById("id-modal-card"),j=document.querySelector(".shop-modal-close-btn");a.listCards.addEventListener("click",D);async function D(t){t.preventDefault();const{target:e}=t,s=e.closest(".filt-div-card");if(!s||e.closest(".filt-btn-card"))return;const r=s.dataset.idcarts;try{const n=await A(r);V(n),I.classList.remove("is-hidden-card")}catch(n){console.log(n)}}j.addEventListener("click",x);function x(){I.classList.add("is-hidden-card")}function H(t){const{target:e}=t,s=e.closest(".shop-btn-card");if(v(),!s)return;const o=s.dataset.idcards;x();const r=localStorage.getItem("cart"),n=JSON.parse(r);if(n){const l=n.find(d=>d._id===o);if(l){l.quantity+=1,localStorage.setItem("cart",JSON.stringify(n));return}n.push({_id:o,quantity:1}),localStorage.setItem("cart",JSON.stringify(n));return}const c=[];c.push({_id:o,quantity:1}),localStorage.setItem("cart",JSON.stringify(c))}a.addCartBtn.addEventListener("click",H);const b={keyword:null},i={keyword:null,category:null,page:1,limit:6};window.addEventListener("resize",v);async function R(){try{const e=(await _()).map(o=>{let r;return o.includes("_")?r=o.replace(/_/g," "):r=o,`<option value="${o}">${r}</option>`}).join("");a.categor.innerHTML+=e;const s=document.createElement("option");s.value="Show all",s.textContent="Show all",a.categor.prepend(s)}catch(t){console.log(t)}}async function v(){const t=G();i.limit=t;const e=JSON.parse(localStorage.getItem("filters"));e&&(i.keyword=e.keyword,i.category=e.category,i.page=e.page,localStorage.setItem("filters",JSON.stringify(i)),O());try{const o=(await J(i.keyword,i.category,"","","",i.page,t)).results;F(o)}catch(s){console.log(s)}}R();v();a.form&&(a.form.addEventListener("input",U),a.form.addEventListener("submit",W));async function U(){const t=a.filtersInput.value;b.keyword=t,t===""&&(b.keyword=null),localStorage.setItem("keyword",JSON.stringify(b))}async function W(t){t.preventDefault();const e=a.filtersInput.value;i.keyword=e,e===""&&(i.keyword=null);const s=a.filtersCategories.value;i.category=s,s===""&&(i.category=null),s==="Show all"&&(i.category=null),localStorage.setItem("filters",JSON.stringify(i)),v(),t.target.reset()}function O(){try{const t=JSON.parse(localStorage.getItem("keyword"));a.filtersInput&&(a.filtersInput.value=t.keyword||"")}catch{return}}O();function G(){const t=window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth;return t>=1440?9:t>=768?8:6}function K(t){const{target:e}=t,s=e.closest(".filt-btn-card");if(!s)return;s.querySelector(".filt-use").setAttribute("href",`${g}#icon-check`),s.setAttribute("disabled","true");const r=s.dataset.idcards,n=localStorage.getItem("cart"),c=JSON.parse(n);if(c){const l=c.find(d=>d._id===r);if(l){l.quantity+=1,localStorage.setItem("cart",JSON.stringify(c));return}c.push({_id:r,quantity:1}),localStorage.setItem("cart",JSON.stringify(c));return}}a.listCards.addEventListener("click",K);function Q(t){return t.category.includes("_")&&(t.category=t.category.replace(/_/g," ")),`
